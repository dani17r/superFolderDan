  ...
  <q-layout view="lHh Lpr lFf">
    <q-header>
      <q-toolbar>
        ...
        <q-toolbar-title> SuperFileDan </q-toolbar-title>
        ...
        <div>
          <q-btn
            ...
            label="Logout"
            no-caps
            @click="dialogs.confirm.toggle()"
          />
          <ConfirmAction
            message="Are you sure to log out ?"
            v-model="dialogs.confirm.status"
            @yes="logout()"
          />
        </div>
      </q-toolbar>
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above bordered>
      <q-list>
        <q-item
          ...
          clickable
          v-ripple
          :to="{ name: 'profile', params: { id: store.auth.current?.id } }"
        >
          ...
          <q-item-section>
            <q-item-label>{{
              store.auth.current?.user_metadata.fullname
            }}</q-item-label>
            <q-item-label caption lines="1">{{
              store.auth.current?.user_metadata.email
            }}</q-item-label>
          </q-item-section>
        </q-item>
