      ...
      <form @submit.prevent.stop="onSubmit" @reset.prevent.stop="onReset">
        ...
        <q-card-section>
          <q-input dense outlined v-model="form.fullname.val.value" label="Full Name" class="q-mt-md"
            :ref="(val) => (form.fullname.refs.value = val)" :rules="form.fullname.rules" clearable />

          <q-input dense outlined v-model="form.birthdate.val.value" mask="date" :rules="form.birthdate.rules"
            class="q-mt-md" clearable :ref="(val) => (form.birthdate.refs.value = val)" placeholder="Date Of Birth">
          ...
          <q-input dense outlined type="email" v-model="form.email.val.value" label="Email" class="q-mt-md"
            :ref="(val) => (form.email.refs.value = val)" :rules="form.email.rules" clearable />

          <q-input dense outlined class="q-mt-md" v-model="form.password.val.value" label="Password"
            :ref="(val) => (form.password.refs.value = val)" :rules="form.password.rules"
            :type="!form.password.view.value ? 'password' : 'text'" clearable>
            <template v-slot:append>
              <q-icon :name="!form.password.view.value ? 'visibility_off' : 'visibility'
                " class="cursor-pointer" @click="form.password.view.value = !form.password.view.value" />
            </template>
          </q-input>
        </q-card-section>

        <q-card-section>
          <q-btn unelevated color="primary" label="SingUp" no-caps class="full-width tw-rounded-lg no-shadow"
            type="submit" />
          <q-btn flat color="primary" label="Reset" no-caps class="full-width tw-rounded-lg no-shadow" type="reset" />
        </q-card-section>

        <q-card-section class="text-center q-pt-none">
          <div class="text-grey-8">
            You want to
            <router-Link tag="a" :to="{ name: 'login' }" class="text-secondary text-weight-bold">login
              ?</router-Link>
          </div>
        </q-card-section>
      </form>
